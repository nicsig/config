...
if exists('loaded') || exists('g:loaded_matchup') || stridx(&rtp, 'vim-matchup') == -1
    finish
endif
var loaded = true
...
g:matchup_matchparen_enabled = false
...
g:matchup_matchparen_nomode = "ivV\<c-v>"
...
#
#     g:matchup_matchparen_offscreen = {method: 'popup', scrolloff: true}
#                                                        ^-------------^
#
...
augroup MyMatchup | au!
    au FileType * SetBufferLocalOptions()
augroup END

def SetBufferLocalOptions()
    setl mps+=“:”,‘:’
    # Rationale:{{{
    #
    ...
    #
    #     b:undo_ftplugin = get(b:, 'undo_ftplugin', 'exe') .. '| set mps< | unlet! b:match_ignorecase b:match_words'
    #
