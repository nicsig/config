...
augroup VirtualTextReplicatePopups | au!
    ...
    # As a  result, the guard  at the top  of `ReplicatePopups()` might  fail to
    # correctly bail out.
    ...
    #}}}
    au WinEnter,BufWinEnter * au SafeState * ++once ReplicatePopups()
augroup END
...
# TODO: Turn this script into a plugin providing importable functions.
# Update the gist with a comment referring to a more up-to-date version.
...
def VirtualTextAdd(props: dict<any>)
    ...
    augroup VirtualTextPersistAfterReload
        au! * <buffer>
        au BufUnload <buffer> SaveTextProperties()
        au BufReadPost <buffer> RestoreTextProperties()
    augroup END
enddef

def SaveTextProperties()
...
def UpdatePadding(buf: number, start: number, ...l: any)
