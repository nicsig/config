...
PGM="$1"
SUPPORTED_PGMS=(mpv tmux vim weechat zsh)
...
elif [[ ! 'mpv tmux vim weechat zsh' =~ (^|[[:space:]])"${PGM}"($|[[:space:]]) ]]; then
...
Edit the script "$(basename "$0")" so that it sets the version of your compiled package correctly.
...
configure() { #{{{2
  ...
  #}}}
    ...
    sed -i "/AC_INIT/s/master/${VERSION}/" configure.ac
    sh autogen.sh
    ./configure
