...
#
# No.
# We source `bashrc` only in interactive shells, but maybe some programs aren't
# started from an interactive shell and still rely on `$PATH` to be properly set.
# Maybe some which are started from the display manager?
#
#     https://unix.stackexchange.com/a/88266/232487
#
# Quote:
...
# No.
#
# Ubuntu  probably  sets  `PATH`  in  `/etc/environment`  or  `/etc/login.defs`,
# because  even when  we  disable all  our  config files,  it  has some  values,
